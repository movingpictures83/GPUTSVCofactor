...
#include "GPUTSVCofactorPlugin.h"

void GPUTSVCofactorPlugin::input(std::string file) {
 inputfile = file;
...
printf("***Cofactor on %d x %d Matrix on GPU***\n",N,N);
coFactor<<<N, N>>>(pA,pC,N);
...
void GPUTSVCofactorPlugin::output(std::string file) {
...
PluginProxy<GPUTSVCofactorPlugin> GPUTSVCofactorPluginProxy = PluginProxy<GPUTSVCofactorPlugin>("GPUTSVCofactor", PluginManager::getInstance());
